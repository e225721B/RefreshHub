...
| **清掃・準備の 15 分は「自分の施術の後」だけを守る** | 空き枠計算（`lib/slots.ts`）で、候補の枠が他の予約と重なるかを判定するときは**施術時間ぶんだけ**で見る（自分の清掃時間は判定に含めない）。ただし保存する `blockEndTime` / `endAt` は従来どおり「施術 + 15 分」のまま（自分の後に来る予約からはきちんと守られる） | 全ベッドが 10:00〜11:00 で埋まっている状態だと、9:45 開始の 15 分（施術は 9:45〜10:00 で終わる）まで「自分の清掃時間が 10:00〜10:15 の予約と重なる」という理由で選べなくなっていた。清掃は**その施術の後**に必要なだけで、直後に別の予約が入っていても本人の施術と時間的に衝突するわけではないため、判定から外した |
