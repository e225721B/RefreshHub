...
| データベース | ~~SQLite~~ → **PostgreSQL（Supabase）** | 当初は「ファイル 1 個で動きサーバ構築が不要」という理由で SQLite を選んだ。**Vercel へデプロイする段階で PostgreSQL（Supabase）に変更した**（2026-09-11）。Vercel の実行環境はリクエストごとに使い捨てで、書き込んだファイルが残らないため、ファイル 1 個で持つ方式が成立しない。手順と移行時の注意は `docs/intern/deploy.md` |
