...
| 2 | 予約画面（上段: 自分のこれからの予約 + キャンセル。下段: 週表示の空き枠） | `/` | 全員 | AC-1〜AC-4 / AC-13 |
...
| **ユーザーの削除は論理削除に統一する**（2026-09-10 レビューで変更） | `/admin/users` の削除は **`User.active` を false にするだけ**。DB の行は消さない。ログインできなくなり空き枠の担当にも出なくなるが、過去の予約・集計はそのまま残り、あとから有効に戻せる。**自分自身と、有効な管理者が 1 人だけのときのその管理者は削除できない** | 当初は「記録が 1 件も無ければ物理削除、あれば無効化」としていた（登録の打ち間違いを消せるようにするため）。**レビューで「後から見返す必要が出てくるので論理削除がよい」と指摘され、条件分岐をやめて論理削除に一本化した。**分岐があると「どちらが起きるか」を画面で説明し続けねばならず、削除できるつもりで押した操作が実は無効化だった、という取り違えも起きる。実績のある人を消すと過去の予約と集計（AC-6 / AC-7）が壊れる点は変わらない（`active` フラグを持つ判断と同じ理由）。管理者が 0 人になると以降だれもアカウントを登録できなくなるため、その操作は禁止する |
| **`/admin/users` に「状態」列を置かない**（2026-09-10 レビューで変更） | 一覧の列は 氏名・メールアドレス・権限・性別・利用実績・操作 の 6 つ。有効・無効は列にせず、**削除済みの行を薄く表示し、操作列に「有効に戻す」ボタンを出す**ことで示す | 論理削除に一本化した結果、**`active = false` になるのは削除したときだけ**になり、「状態」列と操作列が同じことを二重に言うことになった。列を減らして、状態は行の見た目と出るボタンで伝える |
...
| `lib/stats.ts` | 期間の正規化・横軸の粒度判定・集計（`aggregate` は DB に触らない純粋な関数） | AC-6 / AC-7 | C |
