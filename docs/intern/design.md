...
| 3 | 利用者: 自分の予約（これから / 過去のタブ、キャンセル） | `/my-reservations` | 全員 | AC-13 |
| 3a | キャンセル確認（モーダル） | — | 全員 | AC-13 |
| 4 | マッサージ師: 自分の予約状況 | `/therapist` | マッサージ師 | AC-15 |
| 5 | 管理者: 予約状況（ベッド × 時間） | `/admin` | 管理者 | AC-8 / AC-13 |
| 6 | 管理者: 集計 | `/admin/stats` | 管理者 | AC-6 / AC-7 |
| 7 | 管理者: 勤務パターン + 欠勤管理 | `/admin/shifts` | 管理者 | AC-9 |
| 8 | 管理者: ユーザー管理（**アカウント登録**・権限変更・パスワード再設定・無効化） | `/admin/users` | 管理者 | AC-17 |
| 9 | 管理者: ベッド管理 | `/admin/beds` | 管理者 | AC-10 |
| 10 | 管理者: 通知の記録 | `/admin/notifications` | 管理者 | AC-11 / AC-12 |

※ 10 は D-2 の決定に伴う画面。「送られるはずの通知」を目で確認するために必要。
※ 3（自分の予約）は当初 `/` 予約画面の上段に埋め込む予定だったが、実装時に「今の予約」と
「これから新しく予約する」が同じ見た目で並ぶと区別しにくいという指摘を受け、**独立した画面に分けた**。
「これから」「過去」のタブと状態バッジ（予約済み / 利用済み / キャンセル済み）を持つ。
...
> 画面は **10 枚 + モーダル 3 つ**、データは **User / Therapist / Bed / Shift / Reservation / Notification の 6 テーブル**。
