...
| B-2 | 「自分の予約」の表示。**design.md を更新し、`/` 予約画面への埋め込みではなく独立画面 `/my-reservations` にした**（今の予約と新しい予約が同じ見た目で並ぶと分かりにくいという指摘のため）。「これから」「過去」のタブ、状態バッジ（予約済み / 利用済み / キャンセル済み） | 完了 |
| B-3 / B-5 | `/my-reservations` にキャンセルボタン + 確認モーダルを実装。`cancelReservation()` を呼び、`cancelledAt` を保存 | 完了。ブラウザでキャンセル→タブの件数・状態バッジが即座に更新されることを確認 |
| B-4 | 予約確認のモーダル化。備考欄を追加。性別選択は要検討（Issue #7） | 未着手 |
| B-7 | 利用ガイドの文言更新 | 未着手 |

`listMyReservations()` は「これから」だけでなく全件（過去・キャンセル済み含む）を返すように変更し、
画面側でタブに振り分ける形にした（当初の「未来のものだけ返す」設計から変更）。

design.md の画面一覧を更新（9 枚 → **10 枚**、モーダル 2 つ → **3 つ**。`/my-reservations` とキャンセル確認モーダルを追加）。

ブラウザでの確認: ログイン→予約→`/my-reservations`で「これから」に表示→キャンセル→
確認モーダル→確定→「これから」から消え「過去」に「キャンセル済み」で表示、を一通り確認。
過去の予約（`status="booked"`かつ開始時刻が過去）が「利用済み」と表示されることも確認。
