...
| 合計施術回数 | 85 件 | 85 |
| ユニーク利用者数 | 24 人 | 24 |
| 合計施術時間 | 40 時間 45 分 | 2445 分 |
| 1 人あたり | 3.5 回 | 3.5 |
| キャンセル率 | 8%（7 件 = 利用者都合 6 / 運営都合 1） | 7 件 / 6 / 1 / 8% |
| 一覧表の最大の利用回数 | 5 回（同数が複数人。並び順は 回数 → 最終利用日 → 名前） | 5 回 |

上位の人でも 30 日間で 5 回どまりなのは、**週 1 回までのルール**を守るサンプルデータにしたため
（30 日 ≒ 4〜5 週なので、毎週使っても 5 回が上限）。

グラフの棒も、**高さがすべて件数に比例していること**と、**30 本の棒の合計が KPI の合計施術回数と一致すること**を確認した。
...
```
$ npx tsx prisma/seed.ts
投入完了: 管理者 1 名 / マッサージ師 4 名 / 利用者 24 名 / ベッド 3 台
サンプル予約 282 件（2026-06-12〜2026-09-09）: 利用 255 件 / キャンセル 27 件 / ユニーク利用者 24 人
1 人あたりの週の最大予約数: 1 件（福利厚生のルールは週 1 回まで）
```
...
- **1 人あたり週 1 回まで**（福利厚生のルール）を守って作り、シードの最後に守れているかを検算している（破っていたらエラーで止まる）
...
期間を広げると横軸が自動で切り替わる（今年 = 月別）。**月別のときだけ**ユニーク利用者数の棒が出る:

![集計画面（月別）](screens/admin-stats-month.png)
...
# A-2 のレビュー対応（2026-09-10）

PR #12 のレビューで 3 点の指摘を受け、修正した。

## 指摘 1: 日別のユニーク利用者数は計測が必要か

> 1 日に複数回マッサージしたい人はあんまりいない。そもそも福利厚生ルールとして
> **週 1 しか使えない**ことが決まっているので、現状は「確実にユニーク」になるかと思いました。

**指摘のとおり。**週 1 回までなら、**日別でも週別でもユニーク利用者数は必ず予約数と同じ値**になり、
同じ数字を 2 本の棒で描いていたことになる。

| 対応 | 内容 |
|---|---|
| グラフ | **ユニーク利用者数の棒は月別のときだけ出す。**日別・週別は予約数の 1 本にし、棒を太くした。「数字で見る」の表とマウスオーバーの吹き出しからも同様に外した |
| 見出し | 何で区切っているか（日別 / 週別 / 月別）を必ず出す。**棒の本数だけでなく「2 本目を出すかどうか」も決める情報**になったため |
| 日別・週別の説明 | 「週 1 回までのルールがあるため、日別ではユニーク利用者数は予約数と同じ値になります」と画面に書いた |
| サンプルデータ | **週 1 回までを守るように作り直した**（下記） |
| 要件 | 週 1 回までのルールは requirements.md にも design.md にも記録が無かった。**requirements.md の「予約枠のルール」に追記**し、影響範囲と未解決の論点（R-4）も書いた |

**月別だけは 2 本のずれに意味が残る。**同じ人が月に何度も使えるため、
「予約数だけ伸びている＝同じ人が使っている」「2 本一緒に伸びている＝使う人が増えている」が読める。
これは要件「リピーターは二の次。いろんな人に使ってもらいたい」に直接答える指標なので、月別では残した。

### サンプルデータの作り直し

以前のサンプルは**このルールを破っていた**（30 日間で同じ人が 22 回）。
画面の説明と食い違うので、シードを次のように直した。

- 予約を置くとき、**その人がその週（月曜はじまり）にすでに予約していれば置かない**
- 投入の最後に「1 人あたりの週の最大予約数」を数え、**2 件以上ならエラーで止める**
- 週 1 回までだと 10 人では 1 週間に最大 10 件しか立たず、ベッド 3 台の部屋として現実離れするため、
  **利用者を 10 人 → 24 人**に増やした（名前は施術者と同じ「利用者 A」形式の仮名に統一）

### 残した課題（R-4）

**予約時に週 1 回までを止める検証は入れていない。**予約作成の仕様変更になり、
予約画面（B の担当）にも手が入るため、この PR の範囲を超える。requirements.md の R-4 に論点として残した。

## 指摘 2: 「削除」と「状態」の 2 つは冗長ではないか

> 削除されるとき以外、「無効」はありえますか？

**ありえない。**`active = false` になるのは削除操作のときだけなので、
「状態」列と「操作」列が同じことを二重に言っていた。

| 対応 | 内容 |
|---|---|
| `/admin/users` | **「状態」列を削除。**列は 氏名・メールアドレス・権限・性別・利用実績・操作 の 6 つになった |
| 削除済みの行 | **行全体を薄く表示**し、操作列に「有効に戻す」を出すことで状態を示す |
| 削除ボタン | **削除済みの人には出さない。**押しても「すでに削除されています」で断られるだけだった |

## 指摘 3: 物理削除になっていないか（論理削除にしたい）

> テーブルの active カラムで状態管理していたような・・・論理削除ですよね？
> 大体こういうのは後で見返す必要がでてくるので論理削除がいいです。

**なっていた。**`lib/users.ts` の `deleteUserAccount` は
「予約などの記録が 1 件も無ければ `tx.user.delete()` で DB から消す」作りだった
（マッサージ師なら `Therapist` / `TherapistWorkHours` / `TherapistAbsence` も物理削除していた）。

| 対応 | 内容 |
|---|---|
| `deleteUserAccount` | **記録の有無によらず `User.active = false` にするだけ**にした。物理削除のコードを消した |
| 戻り値 | `mode: "deleted" | "deactivated"` の分岐を廃止（起きることが 1 つになったため） |
| 二重削除 | すでに削除済みの人を削除しようとしたら断る |
| 確認ダイアログ | 「完全に削除します。元に戻せません」の分岐を消し、**常に**「ログインできなくなります。過去の予約と集計は残ります。あとから有効に戻せます」と出す |
| 完了メッセージ | 「削除しました」で終わらせず、何が起きたかを書く |
| design.md | 「記録が無ければ物理削除、あれば無効化」の判断を**論理削除に一本化**として書き直し、変更の理由を残した |

### アプリのコードに残っている物理削除

`grep` で確認した結果、**アプリのコードに `delete` / `deleteMany` は 1 つも残っていない。**

| 場所 | 種類 | 判断 |
|---|---|---|
| `prisma/seed.ts` | 全テーブルの `deleteMany` | **残す。**初期データの投入前に作り直すためのもので、運用中の操作ではない |
| `lib/session.ts` | Cookie の `delete` | **残す。**DB ではなくログアウト時の Cookie 削除 |
| `lib/users.test.ts` | テストで作ったユーザーの後始末 | **残す。**テストが自分で作ったデータを消しているだけ |

### テストの入れ替え

物理削除を前提にしていたテスト 2 件を、論理削除を確かめるものに書き換え、1 件足した。

| テスト | 確認すること |
|---|---|
| 記録が 1 件も無いユーザーでも、DB からは消えず active が false になるだけ | 行が残り、`active = false`、そのアカウントでログインできない |
| マッサージ師を削除しても Therapist の行は消えない | 担当した予約を後から辿れる。`Therapist.active`（勤務側の設定）は勝手に触らない |
| 同じユーザーを 2 回削除しようとしたら、2 回目は断る | （新規） |
| 管理者が 2 人以上いれば、片方は削除できる | 行は残り `active = false` になる（以前は「行が消える」を確認していた） |
...
![集計画面・日別](screens/admin-stats.png)

月別（2 系列。ユニーク利用者数の棒が出る）:

![集計画面・月別](screens/admin-stats-month.png)

ユーザー管理（「状態」列なし。最下行が削除済み）:

![ユーザー管理](screens/admin-users.png)
