...
| U-3 | 権限が「マッサージ師」のとき | **性別の選択欄を出す。**性別は `User.gender` に持ち（実装中に `Therapist` から移動。下記「仕様変更」を参照）、**マッサージ師のときだけ必須**。利用者が予約画面で担当の性別を絞り込むために使う。User と Therapist を 1 つのトランザクションで同時に作る |
...
| 2026-09-09 | 性別を **`Therapist.gender` → `User.gender` へ移動**。利用者・管理者も任意で登録でき、マッサージ師のときだけ必須 | design.md（ER 図 第 2 版 / 設計上の判断）、`prisma/schema.prisma`、マイグレーション `20260909090000_move_gender_to_user` | 利用者・管理者にも性別を持たせたいという学生の指示。両方の表に列を作ると同じ情報が 2 か所に分かれるため、`User` 側へ一本化した |
