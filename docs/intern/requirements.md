...
### 利用回数の上限: 1 人あたり週 1 回まで（2026-09-10 追記）

**福利厚生の運用ルールとして、1 人が使えるのは週 1 回まで**と決まっている。
（レビューで判明。ヒアリング時に出ておらず、第 1 版・第 2 版の設計には入っていなかった。）

このルールが効いてくるところ:

| 影響する箇所 | 内容 |
|---|---|
| 集計（AC-6） | **日別・週別では「ユニーク利用者数」が必ず「予約数」と同じ値になる。**同じ人がその週に 2 回予約できないため。差が出るのは月別以上を見たときだけ |
| 規模の試算 | 上の「700 人 × ベッド 3 台」は 1 人 1 回あたりの試算なので、この上限とは矛盾しない。週 1 回まででも供給が上限（3 台）に先に当たる |
| 予約時のチェック | **未実装。**「今週すでに予約がある人は新しく取れない」という検証は入っていない。別タスクとして残す（下記 R-4） |

| # | 論点 | なぜ重要か |
|---|---|---|
| R-4 | **週 1 回までのルールを、予約時にシステムで止めるか / 運用で守るか。**止める場合、週の区切り（月曜はじまり）と、管理者が例外的に入れられるかを決める必要がある | 止めないと、ルールを破った予約が入り、集計の前提（日別・週別はユニーク）も崩れる |
...
| # | 誰が | 何をすると | どうなる | 優先 | 対応する困りごと |
|---|---|---|---|---|---|
| AC-11 | 利用者 | 予約を確定する | **即座にマッサージ師へメールが自動送信される** | 必須 | マッサージ師が予約を把握する手段が不明（As-Is の Q-10） |
