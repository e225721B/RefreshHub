 ...
 *
 * **2026-09-11 以降、このルールは `createReservation`（AC-19）で予約時に止めるようになった**ため、
 * 新規の予約でここが 0 件以外になることは無いはず。0 件以外が出たら、
 * 強制ロジックをすり抜けた予約（AC-19 実装前の既存データ・管理者による直接操作など）が
 * 疑われるので調べること。ここを消さずに残しているのはそのため。
 *
