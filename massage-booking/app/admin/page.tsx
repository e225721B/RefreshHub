...
const BREAKS = WINDOWS.slice(0, -1).map((w, i) => ({
  start: w.end,
...
const SLOTS_PER_BED = WINDOWS.reduce(
  (sum, w) => sum + (w.end - w.start) / STEP_MIN,
  0,
);
...
export default async function AdminPage({
  ...
  const blocks = reservations.map((r) => {
    ...
    return {
      ...
      startMin,
      endMin,
      slots: (endMin - startMin) / STEP_MIN,
    };
  });
  ...
  function blockStartingAt(bedId: string, minute: number) {
    return blocks.find(
      (b) => b.reservation.bedId === bedId && b.startMin === minute,
    );
  }
  ...
  const usageRate =
    capacity === 0 ? 0 : Math.round((usedSlots / capacity) * 100);
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="text-xs leading-snug text-black/60 dark:text-white/60">
                                {r.therapist.user.name} ／ 施術 {r.treatmentMin}{" "}
                                分
                              </div>
                            </td>
                          );
