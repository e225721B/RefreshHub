...
import { GRANULARITY_LABEL, type Granularity, type PeriodPoint } from "@/lib/stats";
 ...
 * **日別・週別ではユニーク利用者数が必ず予約数と同じ値になり、同じ高さの棒を 2 本描くことになる**。
 * （ルールが守られているかは、棒の高さの差ではなく画面上の警告で示す。page.tsx を参照。）
 *
 ...
 *
 * 人数の計算そのものは粒度によらず lib/stats.ts が常に行っている。
 * 将来ルールが見直されて日別でも差が出るようになったら、ここの出し分けを外すだけでよい。
 */
export function PeriodChart({
  ...
  const max = niceMax(
    Math.max(
      0,
      ...points.map((p) => (showUniqueUsers ? Math.max(p.reservations, p.uniqueUsers) : p.reservations)),
    ),
  );
  ...
  return (
    <div>
      {/* 何で区切っているかは、棒の本数と「2 本目を出すかどうか」を決めるので必ず見せる */}
